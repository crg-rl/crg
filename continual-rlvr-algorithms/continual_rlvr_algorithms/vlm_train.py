...
DEFAULT_TASK_RUNNER_CLS = (
    "continual_rlvr_algorithms.method.verl_task_runner:MethodHookTaskRunner"
)
...
def run_vlm_ppo(config: DictConfig, task_runner_cls: type) -> None:
    auto_set_device(config)
    task_runner_class = ray.remote(num_cpus=1)(task_runner_cls)
    run_ppo(config, task_runner_class)


def build_dynamic_vlm_ppo_main():
    @hydra.main(config_path=str(ppo_config_path), version_base=None)
    def main(config):
        run_vlm_ppo(config, resolve_task_runner_cls(config))

    return main


main = build_dynamic_vlm_ppo_main()
