...
exp_name="${EXP_NAME:-qwen2_5vl_7b_crl_tasks_visulogic_spatial_reasoning_osft_grpo_fsdp_vllm_${accel_label}}"
...
tasks="['Assembly','Cross-sectional','Hexahedron','Three Views','Spatial Orders','Polyhedron Rotation']"
num_tasks=6
...
osft_target_preset="${OSFT_TARGET_PRESET:-attn_only}"
...
case "$osft_target_preset" in
    ...
    qkv_only)
        ...
        ;;
    mlp_only)
        target_patterns_override="[mlp.gate_proj,mlp.up_proj,mlp.down_proj]"
        ;;
    *)
...
METHOD=(
    ...
    ++actor_rollout_ref.osft.fsdp2_lazy_init=false
...
if [[ -n "$target_patterns_override" ]]; then
    METHOD+=("++actor_rollout_ref.osft.target_patterns=$target_patterns_override")
fi
...
"$PYTHON_BIN" -m continual_rlvr_algorithms.vlm_train \
    ...
    -- \
    +task_runner_cls=continual_rlvr_algorithms.method.visulogic.task_runner:VisuLogicOSFTRunner \
    "${DATA[@]}" \
    "${MODEL[@]}" \
    "${ACTOR[@]}" \
    "${ROLLOUT[@]}" \
