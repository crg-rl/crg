...
exp_name="${EXP_NAME:-qwen2_5vl_7b_crl_tasks_visulogic_spatial_reasoning_redo_grpo_fsdp_vllm_${accel_label}}"
...
"$PYTHON_BIN" -m continual_rlvr_algorithms.vlm_train \
    ...
    -- \
    +task_runner_cls=continual_rlvr_algorithms.method.visulogic.task_runner:VisuLogicREDORunner \
    "${DATA[@]}" \
    "${MODEL[@]}" \
    "${ACTOR[@]}" \
    "${ROLLOUT[@]}" \
