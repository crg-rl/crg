...
exp_name="${EXP_NAME:-qwen2_5vl_7b_crl_tasks_visulogic_spatial_reasoning_ewc_grpo_fsdp_vllm_${accel_label}}"
...
METHOD=(
    ++ewc.enabled=true
    ++ewc.penalty_coef=1.0
    ++ewc.fisher_decay=1.0
    ++ewc.max_fisher_samples=-1
    ++ewc.eps=1e-8
)
...
"$PYTHON_BIN" -m continual_rlvr_algorithms.vlm_train \
    ...
    -- \
    +task_runner_cls=continual_rlvr_algorithms.method.visulogic.task_runner:VisuLogicEWCRunner \
    "${DATA[@]}" \
    "${MODEL[@]}" \
    "${ACTOR[@]}" \
    "${ROLLOUT[@]}" \
